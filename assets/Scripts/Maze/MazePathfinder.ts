...
export class MazePathfinder {
    ...
    private getOpenNeighbors(idx: number): number[] {
        ...
        if (cell.walls[0] === WallState.NONE && row > 0) result.push(idx - cols);
        if (cell.walls[1] === WallState.NONE && col < cols - 1) result.push(idx + 1);
        if (cell.walls[2] === WallState.NONE && row < rows - 1) result.push(idx + cols);
        if (cell.walls[3] === WallState.NONE && col > 0) result.push(idx - 1);
    ...
    placeWall(row: number, col: number, dir: Dir, state: WallState = WallState.NORMAL) {
        this.setWallState(row, col, dir, state);
        this.recalculate();
    }
    ...
    canPlaceWallSafely(
        ...
        this.setWallState(row, col, dir, WallState.NORMAL);
        this.recalculate();
        ...
        this.setWallState(row, col, dir, WallState.NONE);
        this.recalculate();

        return reachable;
    }

    private setWallState(row: number, col: number, dir: Dir, state: WallState) {
        const idx = row * this.data.cols + col;
        this.data.cells[idx].walls[dir] = state;
        ...
        if (nRow >= 0 && nRow < this.data.rows && nCol >= 0 && nCol < this.data.cols) {
            ...
            this.data.cells[nIdx].walls[OPPOSITE_DIR[dir]] = state;
        }
    }
}